...
function getStudentProfile(account){
 ...
 return Object.assign({bio:'',interests:'Teknologi, jaringan, dan komputer',skills:'TKJ • Networking • Computer',achievement:'Belum ada data',goal:'Mengembangkan kemampuan TKJ',favoriteSubject:'Teknik Komputer & Jaringan',motto:'Belajar dan berkembang bersama.',status:'Aktif'},saved||{});
}
...
function openStudentProfile(name,gender,account){
  ...
  document.getElementById('studentProfileMotto').textContent=profile.motto;
...
function adminRender(tab='overview'){
 ...
 else if(tab==='tasks'){c.innerHTML=`<div class="admin-head-row"><div><h3>Kelola Semua Tugas</h3><p style="color:var(--muted)">Data tugas lokal.</p></div><div class="task-actions"><button class="btn btn-primary" id="adminAddTask">Tambah Tugas</button><button class="mini-btn danger" id="adminDeleteAllTasks">Hapus Semua</button></div></div><div class="admin-task-list">${tasks.map(t=>`<article class="admin-task-row"><div><b>${escapeHTML(t.title)}</b><small>${escapeHTML(t.subject)} • Deadline ${escapeHTML(t.deadline)}</small><span class="admin-task-status">${escapeHTML(t.status)}</span></div><div class="task-actions"><button class="mini-btn" data-cycle-admin-task="${t.id}">Status</button><button class="mini-btn danger" data-delete-admin-task="${t.id}">Hapus</button></div></article>`).join('')||'<div class="empty-state">Belum ada tugas.</div>'}</div>`;document.getElementById('adminAddTask')?.addEventListener('click',adminAddTask);document.getElementById('adminDeleteAllTasks')?.addEventListener('click',()=>{if(confirm('Hapus SEMUA tugas?')){saveTasks([]);renderTasks();adminRender('tasks')}});c.querySelectorAll('[data-cycle-admin-task]').forEach(b=>b.addEventListener('click',()=>{cycleTask(Number(b.dataset.cycleAdminTask));adminRender('tasks')}));c.querySelectorAll('[data-delete-admin-task]').forEach(b=>b.addEventListener('click',()=>{if(confirm('Hapus tugas ini?')){deleteTask(Number(b.dataset.deleteAdminTask));adminRender('tasks')}}));}
...
function adminAddTask(){const title=prompt('Nama tugas?');if(!title)return;const subject=prompt('Mata pelajaran?')||'Umum';const deadline=prompt('Deadline (YYYY-MM-DD)?')||new Date().toISOString().slice(0,10);const desc=prompt('Deskripsi tugas?')||'Tugas ditambahkan dari Control Panel.';const a=loadTasks();a.push({id:Date.now(),title,subject,deadline,status:'todo',desc});saveTasks(a);renderTasks();adminRender('tasks');toast('Tugas berhasil ditambahkan')}
...
(()=>{
 ...
 function updateMobileAccount(u){const label=document.getElementById('mobileAccountLabel');if(label)label.textContent=u?(u.name):'Account / Login';const link=document.getElementById('mobileAccountLink');if(link)link.title=u?('Akun: '+u.name):'Account / Login'}
 function getProfiles(){try{return JSON.parse(localStorage.getItem(profileKey)||'{}')}catch{return {}}}
 function getProfile(u){const p=getProfiles();return Object.assign({bio:'',interests:'Teknologi, jaringan, dan komputer',skills:'TKJ • Networking • Computer',achievement:'Belum ada data',goal:'Mengembangkan kemampuan TKJ',favoriteSubject:'Teknik Komputer & Jaringan',motto:'Belajar dan berkembang bersama.',status:'Aktif'},p[u?.nisn]||{})}
 function saveProfile(u,data){const p=getProfiles();p[u.nisn]=data;localStorage.setItem(profileKey,JSON.stringify(p))}
 function renderAccount(){const u=getSession();syncAdminMenu();if(!u){guest.hidden=false;profile.hidden=true;updateMobileAccount(null);guestTop();return}guest.hidden=true;profile.hidden=false;document.getElementById('profileName').textContent=u.name;document.getElementById('profileName2').textContent=u.name;document.getElementById('profileRole').textContent=u.role==='admin'?'ADMIN • CONTROL PANEL':'SISWA • XI TKJ 1';document.getElementById('profileNisn').textContent=u.role==='admin'?'Akun admin lokal':'NISN: '+u.nisn;document.getElementById('profileLoginValue').textContent=u.role==='admin'?'Username admin':'NISN';const photos=getPhotos(),prof=u.role==='student'?getProfile(u):{};const img=document.getElementById('profileAvatarImg'),fallback=document.getElementById('profileAvatarFallback');if(photos[u.key]){img.hidden=false;fallback.hidden=true;img.onload=()=>{img.hidden=false;fallback.hidden=true};img.src=photos[u.key]}else{img.hidden=true;fallback.hidden=false;fallback.textContent=initials(u.name)}if(topPill){topPill.querySelector('b').textContent=u.name;topPill.title='Akun: '+u.name;const topImg=document.getElementById('topAvatarImg'),topFallback=document.getElementById('topAvatarFallback');if(photos[u.key]){topImg.hidden=false;topFallback.hidden=true;topImg.onload=()=>{topImg.hidden=false;topFallback.hidden=true};topImg.src=photos[u.key]}else{topImg.hidden=true;topFallback.hidden=false;topFallback.textContent=initials(u.name)}}if(u.role==='student'){document.getElementById('accountBio').textContent=prof.bio||'Belum ada bio.';document.getElementById('accountInterests').textContent=prof.interests;document.getElementById('accountSkills').textContent=prof.skills;document.getElementById('accountAchievement').textContent=prof.achievement;document.getElementById('accountGoal').textContent=prof.goal;document.getElementById('accountFavoriteSubject').textContent=prof.favoriteSubject;document.getElementById('accountMotto').textContent=prof.motto;document.getElementById('accountStatus').textContent=prof.status;document.getElementById('accountBioInput').value=prof.bio;document.getElementById('accountInterestsInput').value=prof.interests;document.getElementById('accountSkillsInput').value=prof.skills;document.getElementById('accountAchievementInput').value=prof.achievement;document.getElementById('accountGoalInput').value=prof.goal;document.getElementById('accountFavoriteSubjectInput').value=prof.favoriteSubject;document.getElementById('accountMottoInput').value=prof.motto;document.getElementById('accountStatusInput').value=prof.status}updateMobileAccount(u);renderStudents()}
 ...
 document.getElementById('saveAccountProfile')?.addEventListener('click',()=>{const u=getSession();if(!u||u.role!=='student')return toast('Login sebagai siswa untuk mengubah profil.');saveProfile(u,{bio:document.getElementById('accountBioInput')?.value.trim()||'',interests:document.getElementById('accountInterestsInput')?.value.trim()||'Belum diisi',skills:document.getElementById('accountSkillsInput')?.value.trim()||'TKJ • Networking • Computer',achievement:document.getElementById('accountAchievementInput')?.value.trim()||'Belum ada data',goal:document.getElementById('accountGoalInput')?.value.trim()||'Mengembangkan kemampuan TKJ',favoriteSubject:document.getElementById('accountFavoriteSubjectInput')?.value.trim()||'Teknik Komputer & Jaringan',motto:document.getElementById('accountMottoInput')?.value.trim()||'Belajar dan berkembang bersama.',status:document.getElementById('accountStatusInput')?.value||'Aktif'});renderAccount();toast('Profil berhasil disimpan. Kartu Anggota Kelas ikut diperbarui.')});
 document.getElementById('changePasswordBtn')?.addEventListener('click',()=>{const u=getSession();if(!u||u.role!=='student')return toast('Password admin dikelola dari konfigurasi lokal.');const old=prompt('Password saat ini:');if(old===null)return;const acc=ACCOUNTS.find(x=>x.nisn===u.nisn),stored=getPasswords();if((stored[u.nisn]||acc.defaultPassword)!==old)return toast('Password saat ini salah.');const next=prompt('Password baru:');if(!next||next.length<4)return toast('Password baru minimal 4 karakter.');stored[u.nisn]=next;localStorage.setItem(passKey,JSON.stringify(stored));toast('Password berhasil diubah.')});
 ...
 document.getElementById('changePhotoBtn')?.addEventListener('click',()=>document.getElementById('profilePhotoInput')?.click());document.getElementById('profilePhotoInput')?.addEventListener('change',e=>{const file=e.target.files?.[0],u=getSession();if(!file||!u)return;if(!file.type.startsWith('image/'))return toast('Pilih file gambar.');if(file.size>5*1024*1024)return toast('Foto maksimal 5 MB.');const reader=new FileReader();reader.onload=()=>{const source=new Image();source.onload=()=>{const max=720,scale=Math.min(1,max/Math.max(source.width,source.height)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(source.width*scale));canvas.height=Math.max(1,Math.round(source.height*scale));const ctx=canvas.getContext('2d');ctx.drawImage(source,0,0,canvas.width,canvas.height);const compressed=canvas.toDataURL('image/jpeg',0.82);try{const p=getPhotos();p[u.key]=compressed;localStorage.setItem(photoKey,JSON.stringify(p));renderAccount();renderStudents();e.target.value='';toast('Foto profil tersimpan dan otomatis muncul di Anggota Kelas.')}catch(err){toast('Penyimpanan foto penuh. Coba foto yang lebih kecil.')}};source.src=reader.result};reader.readAsDataURL(file)});
